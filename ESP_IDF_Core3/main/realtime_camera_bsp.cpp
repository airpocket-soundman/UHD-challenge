...
static constexpr bool USE_COLORBAR = false;  // 実機のカメラ映像を表示
...
static camera_config_t camera_cfg = {
    ...
    .pixel_format  = PIXFORMAT_YUV422,  // YUVで受け取り後にRGB変換
...
extern "C" void app_main(void)
{
    ...
    while (true) {
        ...
        // RGB888 -> RGB565 (panel is BGR, also needs byte swap)
        for (size_t i = 0; i < pixels; ++i) {
            uint8_t r = rgb888[i * 3 + 2];  // swap R/B
            uint8_t g = rgb888[i * 3 + 1];
            uint8_t b = rgb888[i * 3 + 0];
            uint16_t rgb565 = ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3);
            conv[i] = __builtin_bswap16(rgb565); // panel expects big-endian
        }
