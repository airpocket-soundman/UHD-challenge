...
def create_single_output_model(input_model, output_model, output_constants=None):
    ...
    if not constants:
        raise ValueError("anchors / wh_scale が見つかりませんでした。_nopost.onnx を指定してください。")

    # Save constants to numpy file (auto-generate path if not specified)
    if output_constants is None:
        output_constants = str(Path(output_model).with_suffix(".npz"))
        print(f"No --constants specified. Auto-generating: {output_constants}")
    ...
    except Exception as e:  # noqa: BLE001 - validation only
...
Example usage:
  ...
  python model_conversion\\create_single_output_model.py \\
    ...
    parser.add_argument(
        "--constants",
        default=None,
        help="Output path for constants (.npz file). If omitted, <output>.npz is used.",
    )

    args = parser.parse_args()

    success = create_single_output_model(args.input, args.output, args.constants)
